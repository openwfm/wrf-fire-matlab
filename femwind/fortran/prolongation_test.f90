...
real, pointer, dimension(:,:,:):: u_m, uc_m, X_m, Y_m, Z_m, cl_z_m, hzc_m ! to read from files
...
call read_array(hzc_m,'hzc')

n = shape(X)     ! fine mesh size
nc = shape(uc)   ! coarse mesh size
