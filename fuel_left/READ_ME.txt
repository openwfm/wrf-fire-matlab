...
1) Generate some data to simulate the fire arrival time at four corners of a grid cell and
   ...
   to generate n samples with 80% of the fire arrival times drawn from a Gaussian distribution with a smaller variance, simulating 
   fires newly arrived at grid cells.
...
The training of a CNN with two hidden layers may be also accomplished with s and r from above by the function train_2_layers.m, with usage

net = train_2_layers(s,r)
