...
3) Train the network using s for the predictors and r as the responses. Training can be done using the GUI in the apps tab for 
   Neural Net Fittin and then selecting Import with s as predictors ans r as responses, using radial button to indicate that
   observations are arranged in rows of the matrices. When data is imported, the green triangle train button will divide the data 
   into training, validation and testing sets and optimize the network.
 
   Optionally, scripts have been generated by Matlab to do the training of the network. Either of the two following scripts will train
   the network if s and r from the steps above are in the workspace.
